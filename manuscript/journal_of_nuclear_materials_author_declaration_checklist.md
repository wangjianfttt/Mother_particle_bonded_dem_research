# Journal of Nuclear Materials author and declaration checklist

This file is a final metadata cross-check for the corresponding author before Editorial Manager upload.

## Author order

1. Jian Wang
2. Siyu Wang
3. Hang Zhang
4. Ming-Zhun Lei
5. Wei Wen
6. Qi-Gang Wu
7. Gang Shen
8. Haishun Deng

## Affiliations

1. Anhui University of Science and Technology, Huainan, Anhui 232001, China
2. Institute of Plasma Physics, Chinese Academy of Sciences, Hefei, Anhui 230031, China

## Author-affiliation mapping

| Author | Affiliation(s) |
| --- | --- |
| Jian Wang | 1, 2 |
| Siyu Wang | 1 |
| Hang Zhang | 1 |
| Ming-Zhun Lei | 2 |
| Wei Wen | 1, 2 |
| Qi-Gang Wu | 2 |
| Gang Shen | 1 |
| Haishun Deng | 1 |

Corresponding author: Jian Wang, `wjfttt@mail.ustc.edu.cn`

## CRediT statement

Jian Wang: Validation, Supervision, Project administration, Methodology, Investigation, Conceptualization.

Siyu Wang: Writing - original draft, Formal analysis, Data curation, Visualization.

Hang Zhang: Writing - original draft, Formal analysis, Data curation, Visualization.

Ming-Zhun Lei: Writing - review and editing, Validation, Project administration, Formal analysis.

Wei Wen: Writing - review and editing, Supervision, Project administration, Methodology, Conceptualization.

Qi-Gang Wu: Validation, Resources, Data curation.

Gang Shen: Writing - review and editing, Supervision, Project administration.

Haishun Deng: Supervision, Project administration.

## Funding declaration

This work was supported by the Anhui Provincial Natural Science Foundation (2408085QA030), the Science Foundation of the Institute of Plasma Physics, Chinese Academy of Sciences (DSJJ-2025-08), the Anhui Intelligent Mine Technology and Equipment Engineering Research Center (AIMTEERC202307), the China Post-doctoral Science Foundation under Grant Number 2024M753266, the Excellent Research and Innovation Team of Anhui Province, China (2022AH010052), the Scientific Research Foundation for High-level Talents of Anhui University of Science and Technology, China (2021yjrc51), and the Collaborative Innovation Program of Hefei Science Center, CAS, China (2019HSC-CIP006).

## Declarations to confirm

- [ ] Author names use the spelling each author wants in the published record.
- [ ] Author order is final.
- [ ] Affiliation mapping is final.
- [ ] Corresponding author email is final.
- [ ] CRediT roles are approved by all authors.
- [ ] Funding numbers are correct.
- [ ] Competing-interest declaration is approved.
- [ ] Generative-AI declaration is approved or edited to match the authors' final disclosure choice.
- [ ] Repository DOI/stable URL inserted after deposit: https://doi.org/10.5281/zenodo.20687351.
- [ ] Editorial Manager metadata matches the manuscript title page.
