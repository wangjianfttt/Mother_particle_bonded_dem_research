...
def add_stage_card(ax: plt.Axes, x0: float, title: str, subtitle: str, accent: str) -> None:
    ax.add_patch(
    ...
    ax.plot([x0 - 0.68, x0 + 0.68], [2.62, 2.62], color=accent, lw=1.1, solid_capstyle="round", zorder=1)
    add_text(ax, x0, 2.73, title, fontsize=6.4, color=accent, fontweight="bold")
...
def draw_template(ax: plt.Axes, x0: float) -> None:
    ...
    add_stage_card(ax, x0, "template", "one crushable\nmother pebble", BLUE)
    ...
    add_text(ax, x0 + 0.48, 2.42, "weak\nplane", fontsize=5.6, color=RED)
    add_text(ax, x0, 0.76, "500 bonded\nsubparticles", fontsize=6.1)


def draw_proxy_bed(ax: plt.Axes, x0: float) -> None:
    add_stage_label(ax, x0 - 0.78, 2.98, "b")
    add_stage_card(ax, x0, "packing", "random bed\nwithout bonds", MUTED)
    ...
    add_text(ax, x0 + 0.33, 2.50, "gravity", ha="left", fontsize=6.3)
    add_text(ax, x0, 0.76, "gravity-settled\nproxy centres", fontsize=6.1)


def draw_locked_insertion(ax: plt.Axes, x0: float) -> None:
    add_stage_label(ax, x0 - 0.78, 2.98, "c")
    add_stage_card(ax, x0, "transfer gate", "zero seating\ndamage", TEAL)
    ax.add_patch(patches.Rectangle((x0 - 0.55, 1.20), 1.10, 1.22, fill=False, edgecolor=INK, linewidth=0.85))
    coords = [
    ...
    add_text(ax, x0 + 0.02, 2.50, "locked insertion", fontsize=6.3, color=TEAL)
    add_text(ax, x0, 0.76, "intact replacement\n493,500 bonds", fontsize=6.1)


def draw_compression(ax: plt.Axes, x0: float) -> None:
    add_stage_label(ax, x0 - 0.78, 2.98, "d")
    add_stage_card(ax, x0, "load-path gate", "spanning native\nforce graph", RED)
    ...
    add_text(ax, x0, 2.50, "compression", fontsize=6.5, color=RED)
    add_text(ax, x0, 0.76, "bond loss only\nduring loading", fontsize=6.1)


def draw_event_database(ax: plt.Axes, x0: float) -> None:
    add_stage_label(ax, x0 - 0.78, 2.98, "e")
    add_stage_card(ax, x0, "event database", "mother-pebble\nchronology", RED)
    ...
    add_text(ax, x0 + 0.02, 2.50, "step -> displacement", fontsize=6.0, color=MUTED)
    add_text(ax, x0 - 0.78, 1.82, "damage\nrank", fontsize=6.6, color=MUTED, rotation=90)
    add_text(ax, x0, 0.76, "assign events to\nid + bed height", fontsize=6.1)
...
def main() -> None:
    ...
    fig, ax = plt.subplots(figsize=(7.35, 2.22))
