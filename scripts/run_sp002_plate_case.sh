...
case_id="${1:-SP-002-contact}"
nsteps="${2:-20000}"
top_vz="${3:--1.0e-4}"
sigma_max="${4:-5.0e7}"
tau_max="${5:-5.0e7}"
bottom_gap_m="${6:-1.0e-6}"
top_gap_m="${7:-1.0e-4}"
z_shift_m="${8:-7.9460215e-6}"
kn_bond="${9:-1.0e14}"
kt_bond="${10:-5.0e13}"
...
template="$case_dir/data/li4sio4_sp_500_nooverlap.multisphere"
...
mkdir -p post
"$lmp" -in in.plate_compression.lmp \
  -var nsteps "$nsteps" \
  -var top_vz "$top_vz" \
