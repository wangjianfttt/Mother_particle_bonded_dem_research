#!/usr/bin/env python3
"""Summarize SP-002 Weibull ensemble samples that have completed."""
...
def read_plan() -> dict[str, dict[str, str]]:
    with (ROOT / "tables/minimal_weibull_strength_sampling_plan.csv").open(newline="") as f:
        return {row["sample_id"]: row for row in csv.DictReader(f)}


def sample_id_from_case(case_id: str) -> str:
    # SP-002-WB-01-x-bulk60p034-weak15p009
...
def main() -> None:
    ...
    for summary_path in sorted((ROOT / "data/processed").glob("SP-002-WB-*_summary.csv")):
    ...
    out = ROOT / "tables/sp002_weibull_ensemble_completed_summary.csv"
    ...
    stats_out = ROOT / "tables/sp002_weibull_ensemble_completed_stats.csv"
    if stats_rows:
